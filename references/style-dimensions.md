...
- 句长：均值只是起点，**标准差和分布形状**才是节奏 ✓（均值/中位数/标准差/p10–p90/分桶）。长句连绵 vs 长短相间 vs 短句连发，是三种完全不同的文体。
- 句法复杂度：每句小句数 ✓（中文以顿号/逗号/分号/冒号分断计数；英文以逗号/分号/冒号加从属标记近似）。这是形合/意合（hypotaxis/parataxis）的操作化近似：英语靠连接手段显性组句（形合），汉语靠语义并置（意合）——连淑能 (1993) 视之为英汉最本质的句法差异（Nida 1982 同此判断）。模仿英文作者的中文译本时尤其要决定：保留形合的长从句，还是拆成意合的流水句。
